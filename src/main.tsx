...
const fallbackRender = ({ error }: { error: Error }) => {
  ...
  return (
    <div className="flex h-screen w-screen flex-col items-center justify-center bg-zinc-950 text-zinc-50 p-4 text-center font-mono">
      ...
      <pre className="text-xs text-red-400 bg-red-950/30 border border-red-900/50 p-4 rounded-lg overflow-auto max-w-2xl max-h-[50vh] text-left whitespace-pre-wrap">
        {error.message}
        {error.stack ? `\n\n${error.stack}` : ""}
      </pre>
